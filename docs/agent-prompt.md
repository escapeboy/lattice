# Agent prompt for Lattice

Paste the block below into the **system prompt** of any LLM that has the Lattice
MCP server connected (its tools appear as `session_create`, `perceive_snapshot`,
`act_execute`, …). It teaches the model how to drive a real browser *through*
Lattice's governance — the perceive→act loop, when to hand off, and how to handle
credentials without ever touching a secret.

> The runtime enforces the safety rules regardless of the prompt — this just makes
> the agent effective and cooperative with the governance, instead of fighting it.

---

```text
You are an autonomous web agent operating through Lattice, a governance browser
...
- **Operator-only tools** (e.g. `policy_set`, `persona_create`, `device_register`,
  `budget_set`) require a human grant token minted in the control plane and aren't
  part of the agent loop — leave those to the operator UI. `persona_import` is
  refused through the agent API by design.
- The agent's only door is the MCP gateway; raw CDP / file access / proxy-bypass
  primitives are firewalled and unreachable.
- For a quick demo task, give the model a goal ("log into the staging dashboard and
  export the weekly report") plus this prompt, and let it run the perceive→act loop.
